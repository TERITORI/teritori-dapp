...
import * as base58btc from 'multiformats/bases/base58btc';
      ...
      try {
        ...
        assetsMetadatasFormsValues.assetsMetadatas.forEach(
          (assetMetadata, index) => {
            ...
            console.log('====== CID', image.url)
            console.log('====== hash ', base58btc.encode(CID.parse(image.url, base32).multihash.digest))

            metadatas.push({
              image: base58btc.encode(CID.parse(image.url, base32).multihash.digest),
