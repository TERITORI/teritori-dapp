...
const getEthereumStandardNFTInfo = async (
  ...
  // NOTE: For bridged NFT, as we cannot fetch nft directly from minter, we have to get it from network config
  // But we will need to fetch the collection info from delivered NFT from the minter
  // So we need to fetch info from both source:
  // - collectionData from NFT which generated by Minter
  // - other info from real deployed NFT
  const nftAddress = await minterClient.callStatic.nft();
  let deployedNFT;

  if (network.idPrefix === "mumbai" || network.idPrefix === "polygon") {
    if (minterContractAddress === network.riotContractAddressGen0) {
      deployedNFT = network.riotBridgedNFTAddressGen0;
    } else if (minterContractAddress === network.riotContractAddressGen1) {
      deployedNFT = network.riotBridgedNFTAddressGen1;
    }

    if (!deployedNFT) {
      throw Error(`Unknown minter: ${minterContractAddress}"`);
    }
  } else {
    deployedNFT = nftAddress;
  }
  ...
  const deployedNftClient = TeritoriNft__factory.connect(deployedNFT, provider);
