...
import { MusicAlbumInfo, GetAllAlbumListRequest } from "../../api/musicplayer/v1/musicplayer";
...
export const useFetchAlbum = (req: GetAllAlbumListRequest) => {
    ...
    useInfiniteQuery(
        ...
        async( {pageParam = req.offset}) => {
            try {
                const postsRequest: GetAllAlbumListRequest = { ...req, offset: pageParam || 0 };
                // Getting Albums
                const musicAlbumList = await getMusicAlbums(selectedNetworkId, postsRequest);
...
const getMusicAlbums = async (networkId: string, req: GetAllAlbumListRequest) => {
    try {
