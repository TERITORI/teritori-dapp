...
import { ExecuteMsg as BunkerExecuteMsg } from "../../contracts-clients/teritori-bunker-minter/TeritoriBunkerMinter.types";
...
export const BatchMingScreen: ScreenFC<"BatchMint"> = ({
  route: {
    params: { id },
  },
}) => {
  ...
  const priceCurrency = getNativeCurrency(networkId, info?.priceDenom);
  const decPrice =
    priceCurrency &&
    Decimal.fromAtomics(info?.unitPrice || "0", priceCurrency.decimals);
  const totalPrice = decPrice?.multiply(Uint64.fromNumber(batchSize));
  ...
  const mint = useCallback(async () => {
    try {
      ...
      const payload: BunkerExecuteMsg = {
        request_mint: { addr: sender },
      };
      ...
      const msgs = range(0, batchSize).map(() => msg);
      console.log("msgs", msgs);
      const res = await cosmwasmClient.signAndBroadcast(sender, msgs, "auto");
      if (isDeliverTxFailure(res)) {
        throw new Error(`Transaction delivery failed: ${res.rawLog}`);
      }
          ...
          >
            ...
            >
              ...
              <TextInputCustom
                label="Batch size"
                name="batchSize"
                value={batchSize ? batchSize.toFixed(0) : ""}
                onChangeText={(text) => {
                  setBatchSize(text ? parseInt(text, 10) : 0);
                }}
              />
              <BrandText>
                Total price:{" "}
