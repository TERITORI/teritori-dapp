...
import {additionalRed, neutral33, neutral77, neutralA3 } from "../../../utils/style/colors";
...
export const PersonalInfoPanel: React.FC<{
  ...
  const onUserPictureChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
  ...
  const onProfilePictureChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    const file = e.target?.files?.[0];
    if (file) {
      uploadFile(file).then((ipfsHash)=>{
    ...
    <View style={{ flexDirection: "column" }}>
      ...
      <View style={{ flexDirection: "column" }}>
        ...
        <View style={{ flexDirection: "row", marginTop: 20 }}>
          <BrandText style={[fontSemibold20, { width: 200 }]}>User Picture</BrandText>
          <View style={styles.picture}>
            ...
            ><input
              ...
              ref={userPictureRef}
            />
              {!seller.avatar && <SVG source={emptySVG} width={"100%"} height={"100%"}/>}
              {seller.avatar && <Image source={{uri:ipfsPinataUrl(seller.avatar)}} style={styles.picture} />}
            </TouchableOpacity>
          </View>
        </View>
        <View style={{ flexDirection: "row", marginTop: 20 }}>
          ...
          <View style={styles.picture}>
            ...
            />
              {!seller.profilePicture && <SVG source={emptySVG} width={"100%"} height={"100%"}/>}
              {seller.profilePicture && <Image source={{uri:ipfsPinataUrl(seller.profilePicture)}} style={styles.picture} />}
            </TouchableOpacity>
          </View>
        </View>
        <View style={{ flexDirection: "row", marginTop: 20 }}>
          <BrandText style={[fontSemibold20, { width: 200 }]}>Description</BrandText>
          <TextInput style={styles.textMultilineInput}
            placeholder={"Share a bit about your work experience, cool projects you've completed, and your area of expertise."} multiline={true}
