use cosmwasm_std::{
    from_json, testing::{mock_dependencies, mock_env, mock_info}, Addr, Attribute, Uint128
};
use cw2981_royalties::{
    msg::{CheckRoyaltiesResponse, Cw2981QueryMsg, RoyaltiesInfoResponse}, ExecuteMsg, Extension, Metadata, Trait
};
use cw721::NftInfoResponse;
use cw721_base::{ContractError, InstantiateMsg, OwnershipError};
use cw_multi_test::{App, ContractWrapper, Executor};

use crate::contract::{entry, Tr721Contract, Tr721ExecuteMsg, Tr721QueryMsg};
...
fn full_flow() {
    ...
    let nft_contract = Box::new(ContractWrapper::new(
        entry::execute,
        entry::instantiate,
        entry::query,
    ));

    let contract_id = app.store_code(nft_contract);
    ...
    // Mint nft with metadata
    {
        let extension = Some(Metadata {
            description: Some("NFT desc".into()),
            name: Some("NFT name".to_string()),
            attributes: Some(vec![Trait {
                display_type: Some("trait display".to_string()),
                trait_type: "trait type".to_string(),
        ...
        let mint_msg = ExecuteMsg::Mint {
            token_id: FIRST_TOKEN_ID.to_owned(),
...
//     // Query nft info
//     {
//         let nft_info_msg = Tr721QueryMsg::NftInfo {
...
//     // Query royalty info
//     {
//         let royalty_info_msg = Tr721QueryMsg::Extension {
...
//         let resp_bin = contract
//             .query(deps.as_ref(), mock_env(), royalty_info_msg)
//             .unwrap();
//         // let resp: RoyaltiesInfoResponse = from_json(resp_bin).unwrap();

//         panic!("{:?}", resp_bin)
//     }

//     // Query check royalties
//     {
//         let check_royalties_msg = Tr721QueryMsg::Extension {
//             msg: Cw2981QueryMsg::CheckRoyalties {},
//         };
//         //  { token_id: "1".to_string(), sale_price: 100 };

//         let resp_bin = contract
//             .query(deps.as_ref(), mock_env(), check_royalties_msg)
//             .unwrap();
//         let resp: CheckRoyaltiesResponse = from_json(resp_bin).unwrap();
