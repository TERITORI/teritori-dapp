use cosmwasm_std::{to_json_vec, Attribute, HexBinary, Uint128};
...
const MERKLE_ROOT: &str = "d4ff464fbe331b9c0efe901fd5dc39dca27428b1520128fa509cb6438d390256";
...
fn full_flow() {
    ...
    // Test merkle tree
    {
        ...
        let proof = MerkleProof::<Sha256>::try_from(proof_bytes_from_hex.to_owned()).unwrap();
    ...
    // Test mint with merkle path
    {
        ...
        let nft_hashes: Vec<[u8; 32]> = nfts
            .iter()
            .map(|x| Sha256::hash(&to_json_vec(x).unwrap()))
            .collect();

        let merkle_tree = MerkleTree::<Sha256>::from_leaves(&nft_hashes);
        let merkle_root = merkle_tree.root().unwrap();
        let root_hex = HexBinary::from(merkle_root).to_string();
