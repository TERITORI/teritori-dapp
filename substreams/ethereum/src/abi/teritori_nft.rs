    ...
    pub mod functions {
        ...
        impl Initialize {
            const METHOD_ID: [u8; 4] = [166u8, 72u8, 124u8, 83u8];
            pub fn decode(
                call: &substreams_ethereum::pb::eth::v2::Call,
            ) -> Result<Self, String> {
                let maybe_data = call.input.get(4..);
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ...
                        ethabi::Token::String(self.contract_uri.clone()),
                    ],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
                encoded.extend(Self::METHOD_ID);
        ...
        pub struct Mint {
            pub receiver: Vec<u8>,
            pub token_id: substreams::scalar::BigInt,
            pub royalty_receiver: Vec<u8>,
            pub royalty_percentage: substreams::scalar::BigInt,
            pub token_uri: String,
        }
        impl Mint {
            const METHOD_ID: [u8; 4] = [181u8, 95u8, 146u8, 178u8];
            pub fn decode(
                call: &substreams_ethereum::pb::eth::v2::Call,
            ) -> Result<Self, String> {
                let maybe_data = call.input.get(4..);
            ...
            pub fn encode(&self) -> Vec<u8> {
                let data = ethabi::encode(
                    &[
                        ...
                        ethabi::Token::Address(
                            ethabi::Address::from_slice(&self.royalty_receiver),
                        ),
                        ethabi::Token::Uint(
                            ethabi::Uint::from_big_endian(
                                self
                        ...
                        ethabi::Token::String(self.token_uri.clone()),
                    ],
                );
                let mut encoded = Vec::with_capacity(4 + data.len());
                encoded.extend(Self::METHOD_ID);
