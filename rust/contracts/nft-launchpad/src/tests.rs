...
mod test {
    ...
    trait Shortcut {
        ...
        fn exec<T: Serialize + Debug>(&mut self, msg: &T) -> AppResponse;
    }

    impl Shortcut for AppContext {
        fn query<T>(&self, msg: &impl Serialize) -> T
        where
    ...
    fn create_default_collection() -> AppContext {
        ...
        app_ctx.exec(&create_collection_msg);
    ...
    fn query_collection() {
        ...
        let resp: Collection = app_ctx.query(&query_collection_msg);
    ...
    #[test]
    fn query_inexist_collection() {
        let app_ctx = get_app_context();

        // Query collection
        let query_collection_msg = QueryMsg::Collection { idx: 2 };
        let resp: Collection = app_ctx.query(&query_collection_msg);
